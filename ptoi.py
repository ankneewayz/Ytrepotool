...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Check if it's a vehicle number format (Indian format: XX00XX0000 or XX00X0000)
    veh_pattern = re.compile(r'^[A-Z]{2}\d{2}[A-Z]{1,2}\d{1,4}$')
    if veh_pattern.match(text.upper()):
...
async def post_init(app):
    ...
    try:
        ...
        logger.error("Telethon failed to start: %s", e)
        logger.warning("Bot will continue without Telethon backend support.")
